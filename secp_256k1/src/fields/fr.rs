...
use k256::{Scalar, elliptic_curve::{generic_array::GenericArray, Field as ScalarField, PrimeField as ScalarPrimeField}};
...
impl From<bool> for Fr {
    fn from(other: bool) -> Self {
        Self::from_repr(<Fr as PrimeField>::BigInt::from(u64::from(other))).unwrap()
    }
}
...
impl From<u128> for Fr {
    fn from(other: u128) -> Self {
        let mut default_int = <Fr as PrimeField>::BigInt::default();
...
impl CanonicalSerializeWithFlags for Fr {
    fn serialize_with_flags<W: ark_std::io::Write, F: Flags>(
        ...
        let output_byte_size = buffer_byte_size(<FrParameters as FpParameters>::MODULUS_BITS as usize + F::BIT_SIZE);

        // Write out `self` to a temporary buffer.
        // The size of the buffer is $byte_size + 1 because `F::BIT_SIZE`
        // is at most 8 bits.
        const BYTE_SIZE: usize = <FrParameters as FpParameters>::MODULUS_BITS as usize / 8;
    ...
    fn serialized_size_with_flags<F: Flags>(&self) -> usize {
        buffer_byte_size(<FrParameters as FpParameters>::MODULUS_BITS as usize + F::BIT_SIZE)
    }
}
...
impl Display for Fr {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        write!(f, stringify!("Secp_256k1 Fr({})"), self.into_repr())
    }
}

impl ark_std::rand::distributions::Distribution<Fr>
    for ark_std::rand::distributions::Standard
{
    #[inline]
    fn sample<R: ark_std::rand::Rng + ?Sized>(&self, rng: &mut R) -> Fr {
        let value = <Scalar as k256::elliptic_curve::Field>::random(rng);
        Fr { value }
    }
}
...
impl Field for Fr {
    ...
    fn characteristic() -> &'static [u64] {
        <FrParameters as FpParameters>::MODULUS.as_ref()
    }
    ...
    fn from_random_bytes_with_flags<F: Flags>(bytes: &[u8]) -> Option<(Self, F)> {
        if F::BIT_SIZE > 8 {
            ...
            let last_limb_mask = (u64::MAX >> <FrParameters as FpParameters>::REPR_SHAVE_BITS).to_le_bytes();
...
impl SquareRootField for Fr {
    fn legendre(&self) -> ark_ff::LegendreSymbol {
        use ark_ff::fields::LegendreSymbol::*;

        let s = self.pow(<FrParameters as FpParameters>::MODULUS_MINUS_ONE_DIV_TWO);
        if s.is_zero() {
            Zero
        } else if s.is_one() {
            QuadraticResidue
...
impl FftField for Fr {
    ...
    fn multiplicative_generator() -> Self {
        Fr::from(<FrParameters as FpParameters>::GENERATOR)
    }
}

impl FpParameters for FrParameters {
    const MODULUS: Self::BigInt = BigInteger256([ 0xbfd25e8cd0364141, 0xbaaedce6af48a03b, 0xfffffffffffffffe, 0xffffffffffffffff ]);
    const MODULUS_BITS: u32 = 256;
    const REPR_SHAVE_BITS: u32 = 0;
    const R: Self::BigInt = todo!();
    const R2: Self::BigInt = todo!();
    const INV: u64 = todo!();
    const GENERATOR: Self::BigInt = BigInteger256( [ 0x7, 0x0, 0x0, 0x0] );
    const CAPACITY: u32 = 255;
    const T: Self::BigInt = todo!();
    const T_MINUS_ONE_DIV_TWO: Self::BigInt = todo!();
    const MODULUS_MINUS_ONE_DIV_TWO: Self::BigInt = BigInteger256( [ 0xdfe92f46681b20a0, 0x5d576e7357a4501d, 0xffffffffffffffff, 0x7fffffffffffffff ] );
}

impl From<num_bigint::BigUint> for Fr {
    #[inline]
    fn from(val: num_bigint::BigUint) -> Fr {
        Fr::from_repr(val.try_into().unwrap()).unwrap()
    }
}

impl Into<num_bigint::BigUint> for Fr {
    #[inline]
...
impl From<BigInteger256> for Fr {
    #[inline]
    fn from(val: BigInteger256) -> Fr {
        Fr::from_repr(val).unwrap()
    }
}

impl Into<BigInteger256> for Fr {
    #[inline]
...
impl PrimeField for Fr {
    ...
    fn from_repr(repr: Self::BigInt) -> Option<Self> {
        ...
        let value = Scalar::from_repr(*bytes_generic);
